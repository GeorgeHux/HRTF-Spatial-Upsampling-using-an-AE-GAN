...
class IterativeBlock(nn.Module):
    def __init__(self, channels, out_channels, kernel, stride, padding, activation='prelu'):
        ...
        self.out_conv = ConvBlock(7 * channels, out_channels, 3, 1, 1, bias=bias, activation=activation, norm=norm)
...
#         self.expansion = expansion
#         self.conv1 = nn.Sequential(
#             spectral_norm(nn.Conv1d(in_channnels, out_channels, kernel_size=3, stride=stride, padding=1)),
#             nn.BatchNorm1d(out_channels),
#         )
#         self.conv2 = nn.Sequential(
#             spectral_norm(nn.Conv1d(out_channels, out_channels * self.expansion, kernel_size=3, padding=1)),
#             nn.BatchNorm1d(out_channels * self.expansion)
#         )
#         self.relu = nn.ReLU()
#         self.prelu = nn.PReLU()
...
#         self.in_channels = 256
#         self.conv1 = nn.Sequential(
#             spectral_norm(nn.Conv1d(nbins, self.in_channels, kernel_size=3, stride=1, padding=1)),
#             nn.BatchNorm1d(self.in_channels),
#             nn.PReLU(),
#         )
#         res_layers = []
...
#             res_layers.append(self._make_layer(block, 512, num_blocks, stride=stride))
#         self.res_layers = nn.Sequential(*res_layers)
#         self.fc = nn.Sequential(spectral_norm(nn.Linear(512 * feature, 512)),
#                                 nn.BatchNorm1d(512),
#                                 # nn.ReLU(True),
#                                 nn.PReLU(),
#                                 # nn.LeakyReLU(0.2, True),
#                                 spectral_norm(nn.Linear(512, latent_dim)))
#
#     def _make_layer(self, block, out_channels, num_blocks, stride=1):
#         downsample = None
#         if stride != 1 or self.in_channels != out_channels:
#             downsample = nn.Sequential(
#                 spectral_norm(nn.Conv1d(self.in_channels, out_channels * self.expansion, kernel_size=1, stride=stride)),
#                 nn.BatchNorm1d(out_channels * self.expansion)
#             )
#         layers = []
#         layers.append(block(self.in_channels, out_channels, stride, self.expansion, downsample))
...
#
#         self.fc = nn.Sequential(
#             spectral_norm(nn.Linear(latent_dim, 512 * 16)),
#             nn.BatchNorm1d(512 * 16),
#             nn.ReLU(True),
#             # nn.PReLU(),
#             Reshape(-1, 512, 16),
...
class Discriminator(nn.Module):
    def __init__(self, nbins: int) -> None:
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, True),
            nn.Conv1d(1024, 1024, kernel_size=3, padding=1, stride=2, bias=False),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, True),
            # nn.Conv1d(1024, 2048, kernel_size=3, padding=1, stride=1, bias=False),
            # nn.BatchNorm1d(2048),
            # nn.LeakyReLU(0.2, True),
            # nn.Conv1d(2048, 2048, kernel_size=3, padding=1, stride=2, bias=False),
            # nn.BatchNorm1d(2048),
            # nn.LeakyReLU(0.2, True),
...
#         self.features = nn.Sequential(
#             # input size: nbins x 529     484
#             spectral_norm(nn.Conv1d(self.nbins, 64, kernel_size=3, padding=1, stride=1, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             spectral_norm(nn.Conv1d(64, 64, kernel_size=3, padding=1, stride=1, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             spectral_norm(nn.Conv1d(64, 64, kernel_size=3, padding=1, stride=2, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             # nbins x 265         242
#             spectral_norm(nn.Conv1d(64, 128, kernel_size=3, padding=1, stride=1, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             spectral_norm(nn.Conv1d(128, 128, kernel_size=3, padding=1, stride=2, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             # nbins x 133      121
#             spectral_norm(nn.Conv1d(128, 256, kernel_size=3, padding=1, stride=1, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             spectral_norm(nn.Conv1d(256, 256, kernel_size=3, padding=1, stride=2, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             # nbins x  67     61
#             spectral_norm(nn.Conv1d(256, 512, kernel_size=3, padding=1, stride=1, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             spectral_norm(nn.Conv1d(512, 512, kernel_size=3, padding=1, stride=2, bias=False)),
#             nn.LeakyReLU(0.2, True),
#             # nbins x 34   31
#         )
#
#         self.classifier = nn.Sequential(
#             spectral_norm(nn.Linear(512 * 31, 512)),
#             nn.LeakyReLU(0.2, True),
#             nn.Linear(512, 1)
#         )
#
...
if __name__ == '__main__':
    ...
    G = AutoEncoder(nbins=256, in_degree=4, latent_dim=128, base_channels=64, num_features=512, out_degree=21)
    x = G(x)
    print(x.shape)
    D = Discriminator(256)
    x = D(x)
